import type { RegistryItemCssVarsSchema } from "../registry/types";
...
export async function updateCssVars(
	cssVars: RegistryItemCssVarsSchema | undefined,
	config: Config,
	options: {
	...
	const cssVarsSpinner = spinner(`Updating ${highlighter.info(cssFilepathRelative)}`, {
...
function updateCssVarsPlugin(cssVars: RegistryItemCssVarsSchema) {
	return {
...
function removeConflictVars(root: Rule | Root) {
	const rootRule = root.nodes.find((node): node is Rule => node.type === "rule" && node.selector === ":root");

	if (rootRule) {
		const propsToRemove = ["--background", "--foreground"];

		rootRule.nodes
			.filter((node): node is postcss.Declaration => node.type === "decl" && propsToRemove.includes(node.prop))
...
function cleanupDefaultNextStylesPlugin() {
	return {
		...
		Once(root: Root) {
			const bodyRule = root.nodes.find((node): node is Rule => node.type === "rule" && node.selector === "body");
			if (bodyRule) {
			...
			const darkRootRule = root.nodes.find(
				(node): node is Rule => node.type === "atrule" && node.params === "(prefers-color-scheme: dark)"
			);
...
function addOrUpdateVars(baseLayer: AtRule, selector: string, vars: Record<string, string>) {
	let ruleNode = baseLayer.nodes?.find((node): node is Rule => node.type === "rule" && node.selector === selector);
