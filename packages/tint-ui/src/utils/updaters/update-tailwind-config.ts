import type { RegistryItemTailwindSchema } from "../registry/types";
...
export async function updateTailwindConfig(
	tailwindConfig: RegistryItemTailwindSchema["config"] | undefined,
	config: Config,
	options: {
		silent?: boolean;
	}
	...
	const tailwindSpinner = spinner(`Updating ${highlighter.info(tailwindFileRelativePath)}`, {
...
export async function transformTailwindConfig(input: string, tailwindConfig: UpdaterTailwindConfig, config: Config) {
...
async function addTailwindConfigTheme(configObject: ObjectLiteralExpression, theme: UpdaterTailwindConfig["theme"]) {
...
function addTailwindConfigPlugin(configObject: ObjectLiteralExpression, plugin: string) {
...
export async function _createSourceFile(input: string, config: Config | null) {
	const dir = await fs.mkdtemp(path.join(tmpdir(), "tint-ui-"));
	const resolvedPath = config?.resolvedPaths?.tailwindConfig || "tailwind.config.ts";
	const tempFile = path.join(dir, `tint-ui-${path.basename(resolvedPath)}`);
	...
	for (let i = 0; i < properties.length; i++) {
		...
		if (prop.isKind(SyntaxKind.PropertyAssignment)) {
			...
			if (initializer && initializer.isKind(SyntaxKind.StringLiteral)) {
				...
				unnestSpreadElements(initializer.asKindOrThrow(SyntaxKind.ArrayLiteralExpression));
			}
		}
	}
}

export function unnestSpreadElements(arr: ArrayLiteralExpression) {
	const elements = arr.getElements();
	for (let j = 0; j < elements.length; j++) {
		const element = elements[j];
		if (element.isKind(SyntaxKind.ObjectLiteralExpression)) {
			...
			// Recursive check on nested arrays
			unnestSpreadElements(element.asKindOrThrow(SyntaxKind.ArrayLiteralExpression));
...
export function buildTailwindThemeColorsFromCssVars(cssVars: Record<string, string>) {
